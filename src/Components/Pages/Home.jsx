...
const Home = () => {
    ...
    const initialForm = { todoId: '', todoTitle: '', todoDesc: '', todoAlloted: '', todoTags: '', todoCreatedAt: ''  };

    const toggleModal = e => {
        setModalOpen(modalOpen.status === false ? {...modalOpen, status: true, data: { ...modalOpen.data, ...initialForm }} : {...modalOpen, status: false, data: { ...modalOpen.data, ...initialForm }})
    }
    ...
    return (
        <div id={colorId}>
            <nav className='navbar flex-center' id={colorId}>
                ...
                { theme === 'dark' ? <FaRegSun className='theme-btn' onClick={e => {
                    storeInLocalStorage('light');
                    setTheme('light');
                }} /> : <BsMoonStarsFill className='theme-btn' onClick={e => {
            ...
            <div className='container flex flex-col align-center justify-start'>
                ...
                <button className='btn add-todo flex-center' id={theme === 'light' ? 'white-color' : 'black-color'} style={{ transform: `rotate(${rotation}deg)` }} onClick={toggleModal}>
                    <AiOutlinePlus />
                </button>
            </div>
        </div>
