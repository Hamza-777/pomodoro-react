...
const Todo = ({ todo: { id, title, desc, tags, alloted, createdAt } }) => {
    ...
    return (
        <div className={`todo flex flex-col align-start justify-center ${themeClass}`}>
            <Link to={`/todo/${id}`} className='todo-title'>{title}</Link>
            {
                desc && <p className='todo-desc'>{desc.charAt(0).toUpperCase() + desc.slice(1)}</p>
            }
            <div className='todo-tags flex-row-wrap align-center justify-start'>
                <h5>TAGS: </h5>
                {
                    tags.filter(tag => tag !== '').length > 0 ? tags.map((tag, idx) => (
